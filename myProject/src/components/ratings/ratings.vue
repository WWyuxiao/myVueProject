<template>
  <div class="ratings" ref="ratingsContent">
    <div class="ratings-content">
      ...
      <split></split>
      <ratingselect :select-type="selectType" :only-content="onlyContent" :desc="desc" :ratings="ratings" @selRatings="filterRatings" @isContent="isContent"></ratingselect>
          ...
          <li v-show="needShow(rating.rateType, rating.text)" v-for="(rating, index) in ratings" :key="index" class="rating-item border-1px">
            ...
            <div class="content">
              ...
              <div class="recommend" v-show="rating.recommend.length">
                ...
                <span class="item" v-for="(item, index) in rating.recommend" :key="index" v-show="rating.recommend && rating.recommend.length">{{item}}</span>
              </div>
    ...
    created () {
      this.$http.get('/api/ratings').then((response) => {
        ...
        if (response.errno === ERR_OK) {
          ...
          this.$nextTick(() => {
            if (!this.scroll) {
              this.scroll = new BScroll(this.$refs.ratingsContent, { // 初始化
                click: true
              })
            } else {
              this.scroll.refresh()
